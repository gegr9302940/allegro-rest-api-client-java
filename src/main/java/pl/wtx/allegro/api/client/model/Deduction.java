...
public class Deduction {
  ...
  public Deduction id(@javax.annotation.Nonnull UUID id) {
    this.id = id;
    return this;
  }
   ...
   * @throws IOException if the JSON Element is invalid with respect to Deduction
   */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!Deduction.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!Deduction.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `Deduction` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }

      // check to make sure all required properties/fields are present in the JSON string
      for (String requiredField : Deduction.openapiRequiredFields) {
